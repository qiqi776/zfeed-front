...
const defaultClassName = "mt-6 rounded-2xl border border-white/40 bg-white/35 px-4 py-4 text-[14px] leading-7 text-on-surface-variant";
...
export function PageState({ state, title, description, actionHref, actionLabel, className = defaultClassName }: PageStateProps) {
    return createElement("div", {
        ...
        title ? createElement("div", { className: "font-label-sm text-on-surface" }, title) : null,
        title
            ? createElement("p", { className: "mt-1 text-[14px] leading-7 text-on-surface-variant" }, description)
